...
int get_serial_num_from_chipid(char* serial)
{
	...
	sunxi_serial[0] = sunxi_soc_chipid[3] | 0xFFFFC000;            /*bit14-bit31-->bit14-bit31*/
	sunxi_serial[0] |= (sunxi_soc_chipid[2] >> 31) << 13;          /*bit31      -->bit13      */
	sunxi_serial[0] |= ((sunxi_soc_chipid[2] & 0x01FFF000) >> 12); /*bit12-bit24-->bit0-bit12*/
	sunxi_serial[1] = sunxi_soc_chipid[1];
	sunxi_serial[2] = (sunxi_soc_chipid[2] & 0xFFF);      /*bit0-bit11-->bit0-bit11*/
	sunxi_serial[2] |= (sunxi_soc_chipid[0] & 0x3) << 12; /*bit0-bit1-->bit12-bit13*/
	...
	sunxi_soc_chipid[0] = smc_readl(SID_BASE + 0x200);
	sunxi_soc_chipid[1] = smc_readl(SID_BASE + 0x200 + 0x4);
	sunxi_soc_chipid[2] = smc_readl(SID_BASE + 0x200 + 0x8);
	sunxi_soc_chipid[3] = smc_readl(SID_BASE + 0x200 + 0xc);
