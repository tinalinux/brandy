/*
 * drivers/video_sunxi/sunxi_v1/de_bsp/iep/deu/iep_deu.c
 *
...
#else
	g_strtab_addr = OSAL_PhyAlloc(512);
#endif
...
#else
	deu_reg_bak[sel] = (__u32)OSAL_PhyAlloc(sizeof(__u32)*0x60);
#endif
	if(deu_reg_bak[sel]) {
		for(i=0; i<0x60; i+=4) {
			//save register
...
__s32 iep_deu_resume (__u32 sel)//restore register
{
	...
	if(deu_reg_bak[sel]) {
		...
		deu_reg_bak[sel] = 0;
#else
		OSAL_PhyFree((void *)deu_reg_bak[sel], sizeof(__u32)*0x60);
#endif
	}
