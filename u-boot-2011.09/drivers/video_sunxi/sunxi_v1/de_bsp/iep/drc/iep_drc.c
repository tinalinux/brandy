...
__s32 drc_clk_init(__u32 sel)
{
	...
	if(!sel) {
		...
		mclk_div = 1;
#if defined(CONFIG_ARCH_SUN8IW3P1) || defined(CONFIG_ARCH_SUN8IW5P1) || defined(CONFIG_ARCH_SUN8IW6P1) || defined(CONFIG_ARCH_SUN8IW8P1)||defined (CONFIG_ARCH_SUN8IW7P1)
		while((pll_freq / mclk_div) > 200000000) {
			mclk_div ++;
		}
#else
...
#else
	drc_reg_bak[sel] = (__u32)OSAL_PhyAlloc(sizeof(__u32)*0x3fc);
#endif
	if(drc_reg_bak[sel]) {
		for(i=0; i<0x3fc; i+=4)	{
			/*  save register */
