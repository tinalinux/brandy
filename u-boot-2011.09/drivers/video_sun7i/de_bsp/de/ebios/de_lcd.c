/*
 * drivers/video_sun7i/de_bsp/de/ebios/de_lcd.c
 *
...
__s32 lvds_open(__u32 sel, __panel_para_t * panel)
{
	...
	else
	{
		...
		bsp_disp_lcd_delay_us(5); //1200ns
...
__s32 tcon_init(__u32 sel)
{
	...
	lcd_dev[sel]->tcon_mul_ctl.bits.dsi_src = 1;
	return 0;
}
...
{
    if((panel->lcd_if == LCD_IF_HV) || (panel->lcd_if == LCD_IF_EDP) || (panel->lcd_if == LCD_IF_EXT_DSI))
	{
...
__s32 tcon1_cfg(__u32 sel,__disp_timing_t* timing)
{
	...
	start_delay = (timing->ver_total_time - timing->ver_pixels)/(timing->interlace+1) - 5;
	start_delay = (start_delay > 31)? 31:start_delay;
	lcd_dev[sel]->tcon1_ctl.bits.start_delay = start_delay;
	return 0;
}
__s32 tcon1_tv_cfg(__u32 sel,__disp_timing_t* timing)
{
	__u32 start_delay;
	lcd_dev[sel]->tcon1_basic0.bits.x = timing->hor_pixels-1;
	lcd_dev[sel]->tcon1_basic0.bits.y = timing->ver_pixels/(timing->interlace+1)-1;
	...
	lcd_dev[sel]->tcon1_basic2.bits.yo = timing->ver_pixels/(timing->interlace+1) + (timing->trd_mode? timing->vactive_space:0)-1;
	lcd_dev[sel]->tcon1_basic3.bits.ht = timing->hor_total_time-1;
	lcd_dev[sel]->tcon1_basic3.bits.hbp = timing->hor_back_time-1;
	lcd_dev[sel]->tcon1_basic4.bits.vt = timing->ver_total_time;
	lcd_dev[sel]->tcon1_basic4.bits.vbp = timing->ver_back_porch-1;
	lcd_dev[sel]->tcon1_basic5.bits.hspw = timing->hor_sync_time-1;
	lcd_dev[sel]->tcon1_basic5.bits.vspw = timing->ver_sync_time-1;
	lcd_dev[sel]->tcon1_io_pol.bits.io0_inv = timing->ver_sync_polarity;
	lcd_dev[sel]->tcon1_io_pol.bits.io1_inv = timing->hor_sync_polarity;
    ...
    switch(mode)
    {
       ...
       case DISP_TV_MOD_PAL_SVIDEO:
        ...
        case DISP_TV_MOD_PAL_NC_SVIDEO:
            ...
            cfg.hor_pixels       = 720;
            cfg.ver_pixels       = 576;
            cfg.hor_total_time       = 864;
            cfg.hor_back_time      = 137;
            cfg.ver_total_time = 625;
            cfg.ver_back_porch      = 20;
            cfg.ver_sync_time     = 2;
            cfg.hor_sync_time     = 2;  
            break;
        ...
        case DISP_TV_MOD_PAL_M_SVIDEO:
            ...
            cfg.hor_pixels       = 720;
            cfg.ver_pixels       = 480;
            cfg.hor_total_time       = 858;
            cfg.hor_back_time      = 116;
            cfg.ver_total_time       = 525;
            cfg.ver_back_porch      = 16;
            cfg.ver_sync_time     = 2;
            cfg.hor_sync_time     = 2;
            break;
#else		// sun5i
       ...
       case DISP_TV_MOD_PAL_SVIDEO:
        ...
        case DISP_TV_MOD_PAL_NC_SVIDEO:
            ...
            cfg.ver_pixels       = 576;
            cfg.hor_total_time       = 864;
            cfg.hor_back_time      = 139;
            cfg.ver_total_time    =  625;
            cfg.ver_back_porch    = 22;
            cfg.ver_sync_time     = 2;
            cfg.hor_sync_time     = 2;
			cfg.ver_sync_polarity = 0;
            break;
        ...
        case DISP_TV_MOD_PAL_M_SVIDEO:
            ...
            cfg.ver_pixels       = 480;
            cfg.hor_total_time       = 858;
            cfg.hor_back_time      = 118;
            cfg.ver_total_time       = 525;
            cfg.ver_back_porch      = 18;
            cfg.ver_sync_time     = 2;
            cfg.hor_sync_time     = 2;
			cfg.ver_sync_polarity = 0;
            break;
        ...
        case DISP_TV_MOD_720P_50HZ:
       	cfg.interlace   = 0;
            cfg.hor_pixels       = 1280;
            cfg.ver_pixels       = 720;
            cfg.hor_total_time       = 1980;
            cfg.hor_back_time      = 258;
            cfg.ver_total_time       = 1500;
            cfg.ver_back_porch      = 22;
            cfg.ver_sync_time     = 2;
            cfg.hor_sync_time     = 2; 
            break;
...
__s32 tcon1_set_vga_mode(__u32 sel, __disp_vga_mode_t mode)
{
	...
	switch(mode)
	{
	        case DISP_VGA_H640_V480:
                        cfg.hor_pixels = 640;//HA
                        cfg.ver_pixels = 480;//VA
                        cfg.hor_total_time        = 0x320;//HT-1=-1
                        cfg.hor_back_time      = 0x90;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 0x41a;//VT*2=*2
                        cfg.ver_back_porch      = 0x22;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 0x2;//VS-1=-1
                        cfg.hor_sync_time     = 0x60;//HS-1=-1
		        break;
	        case DISP_VGA_H800_V600:
                        cfg.hor_pixels = 800;//HA
                        cfg.ver_pixels = 600;//VA
                        cfg.hor_total_time       = 0x420;//HT-1=-1
                        cfg.hor_back_time      = 0xd8;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 0x4e8;//VT*2=*2
                        cfg.ver_back_porch      = 0x1a;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 0x4;//VS-1=-1
                        cfg.hor_sync_time     = 0x80;//HS-1=-1
		        break;
	        case  DISP_VGA_H1024_V768:
                        cfg.hor_pixels = 1024;
                        cfg.ver_pixels = 768;
                        cfg.hor_total_time       = 1344;//HT-1=1344-1
                        cfg.hor_back_time      = 296;//HS+HBP-1=136+160-1
                        cfg.ver_total_time       = 1612;//VT*2=806*2
                        cfg.ver_back_porch      = 34;//VS+VBP-1=6+29-1
                        cfg.ver_sync_time     = 6;//VS-1=6-1
                        cfg.hor_sync_time     = 136;//HS-1=136-1
                        break;
	        case  DISP_VGA_H1280_V1024:
                        cfg.hor_pixels = 1280;//HA
                        cfg.ver_pixels = 1024;//VA
                        cfg.hor_total_time       = 0x698;//HT-1=-1
                        cfg.hor_back_time      = 0x168;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 0x854;//VT*2=*2
                        cfg.ver_back_porch      = 0x28;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 0x3;//VS-1=-1
                        cfg.hor_sync_time     = 0x70;//HS-1=-1
		        break;
	        case  DISP_VGA_H1360_V768:
                        cfg.hor_pixels = 1360;//HA
                        cfg.ver_pixels = 768;//VA
                        cfg.hor_total_time       = 0x700;//HT-1=-1
                        cfg.ver_back_porch      = 0x170;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 0x636;//VT*2=*2
                        cfg.ver_back_porch      = 0x17;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 0x6;//VS-1=-1
                        cfg.hor_sync_time     = 0x70;//HS-1=-1
		        break;
	        case  DISP_VGA_H1440_V900:
                        cfg.hor_pixels = 1440;//HA
                        cfg.ver_pixels = 900;//VA
                        cfg.hor_total_time       = 0x770;//HT-1=-1
                        cfg.ver_back_porch      = 0x180;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 0x74c;//VT*2=*2
                        cfg.ver_back_porch      = 0x1e;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 0x6;//VS-1=-1
                        cfg.hor_sync_time     = 0x98;//HS-1=-1
		        break;
	        case  DISP_VGA_H1680_V1050:
                        cfg.hor_pixels = 1680;//HA
                        cfg.ver_pixels = 1050;//VA
                        cfg.hor_total_time       = 2240;//HT-1=-1
                        cfg.ver_back_porch      = 464;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 2178;//VT*2=*2
                        cfg.ver_back_porch      = 35;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 6;//VS-1=-1
                        cfg.hor_sync_time     = 176;//HS-1=-1
		        break;
	        case  DISP_VGA_H1920_V1080_RB:
                        cfg.hor_pixels = 1920;//HA
                        cfg.ver_pixels = 1080;//VA
                        cfg.hor_total_time       = 2017;//HT-1=-1
                        cfg.ver_back_porch      = 63;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 2222;//VT*2=*2
                        cfg.ver_back_porch      = 27;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 5;//VS-1=-1
                        cfg.hor_sync_time     = 32;//HS-1=-1
		        break;

	        case  DISP_VGA_H1920_V1080://TBD
                        cfg.hor_pixels = 1920;//HA
                        cfg.ver_pixels = 1080;//VA
                        cfg.hor_total_time       = 2200;//HT-1=-1
                        cfg.ver_back_porch      = 148+44;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 1125*2;//VT*2=*2
                        cfg.ver_back_porch      = 36+5;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 5;//VS-1=-1
                        cfg.hor_sync_time     = 44;//HS-1=-1	
                        cfg.hor_sync_polarity   = 1;
			    cfg.ver_sync_polarity   = 1;
		        break;
	        case  DISP_VGA_H1280_V720://TBD
                        cfg.hor_pixels = 1280;//HA
                        cfg.ver_pixels= 720;//VA
                        cfg.hor_total_time       = 1650;//HT-1=-1
                        cfg.ver_back_porch      = 220+40;//HS+HBP-1=+-1
                        cfg.ver_total_time       = 750*2;//VT*2=*2
                        cfg.ver_back_porch      = 5+20;//VS+VBP-1=+-1
                        cfg.ver_sync_time     = 5;//VS-1=-1
                        cfg.hor_sync_time     = 40;//HS-1=-1	
                        cfg.hor_sync_polarity   = 1;
			    cfg.ver_sync_polarity   = 1;
		        break;
	        default:
	...
	cfg.io0_output_tri_en=0;
	cfg.io1_output_tri_en=0;
	cfg.b_remap_if      = 1;
	cfg.interlace=0;
	cfg.trd_mode=0;
	lcd_dev[sel]->tcon1_io_pol.bits.io0_inv = 0;
	lcd_dev[sel]->tcon1_io_pol.bits.io1_inv = 0;
	lcd_dev[sel]->tcon_gctl.bits.io_map_sel = cfg.b_remap_if;
	lcd_dev[sel]->tcon1_io_tri.bits.io0_output_tri_en = cfg.io0_output_tri_en;
	lcd_dev[sel]->tcon1_io_tri.bits.io1_output_tri_en = cfg.io1_output_tri_en;
	start_delay = (cfg.ver_total_time - cfg.ver_pixels)/(cfg.interlace+1) - 5;
	start_delay = (start_delay > 31)? 31:start_delay;
	lcd_dev[sel]->tcon_gint1.bits.tcon1_line_int_num = start_delay+2;
	tcon1_tv_cfg(sel, &cfg);
        return 0;
}
