/*
 * drivers/video_sun7i/de_bsp/de/disp_clk.c
 *
...
__hdle h_lvdsmclk,h_tev0_clk;	//only for reset
...
__s32 lcdc_clk_init(__u32 screen_id)
{
	...
	if(screen_id == 0) {
		...
		h_tev0_clk = OSAL_CCMU_OpenMclk(AHB_CLK_TVE0);
...
__s32 tve_clk_init(__u32 screen_id)
{
	...
	if(screen_id == 0) {
		...
		OSAL_CCMU_MclkReset(h_tev0_clk,RST_INVAILD);
					...
					__u32 lcd_clk_div, __u32 hdmi_freq, __u32 pll_2x, __u32 type)
{
	...
	/*  tv/vga/hdmi */
		...
		lcdmclk1_div = pre_scale;
		hdmiclk_div = (pll_freq_used + (hdmi_freq / 2)) / hdmi_freq;

		h_lcdmclk1 = (screen_id == 0)?h_lcd0ch1mclk1 : h_lcd1ch1mclk1;
		h_lcdmclk2 = (screen_id == 0)?h_lcd0ch1mclk2 : h_lcd1ch1mclk2;
