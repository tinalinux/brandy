...
static int __usb_write_fifo(uchar *buffer, unsigned int buffer_size)
{
    ...
    while(left)
    {
        ...
        transfered += this_len;
        left -= this_len;
        debug("-left=%d , this_len = %d,transfered=%d\n",left, this_len,transfered);
...
static int eprx_recv_op(void)
{
    ...
    else
    {
        if(USBC_Dev_IsReadDataReady(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX))
        {
            ...
            sunxi_usb_dbg("irq:eprx_recv_op fastboot_data_flag=%d,rx_ready_for_data = %d, this_len = %d\n", fastboot_data_flag,sunxi_ubuf.rx_ready_for_data,this_len);
#ifdef CONFIG_FASTBOOT
