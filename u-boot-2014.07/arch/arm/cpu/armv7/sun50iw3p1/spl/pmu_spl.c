...
axp_contrl_info axp1506_ctrl_tbl[] = { \
...
int pmu_init(u8 power_mode)
{
	...
	if(pmu_bus_read(RSB_RUNTIME_ADDR, PMU_IC_TYPE, &pmu_type))
	{
...
int probe_power_key(void)
{
	...
	if(pmu_bus_read(RSB_RUNTIME_ADDR, PMU_IRQ_STATU2, &reg_value))
	{
		return -1;
	}
	...
	if(reg_value)
	{
		if(pmu_bus_write(RSB_RUNTIME_ADDR, PMU_IRQ_STATU2, reg_value))
		{
			return -1;
		}
	}
...
{
	int i = 0;
	int size = ARRAY_SIZE(axp1506_ctrl_tbl);
	for(i = 0; i < size; i++)
	{
		if(!strncmp(name, axp1506_ctrl_tbl[i].name,strlen(axp1506_ctrl_tbl[i].name)))
		{
			break;
		}
	}
