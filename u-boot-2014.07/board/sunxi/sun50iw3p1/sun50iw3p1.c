...
int board_init(void)
{
	...
	if (uboot_spare_head.boot_data.work_mode != WORK_MODE_USB_PRODUCT)
	{
		...
		reg_val = readl(SRAM_CONTRL_REG1);
		reg_val &= ~(0x1<<24);
		writel(reg_val, SRAM_CONTRL_REG1);

		//VE gating :brom set this bit, but not require now
		reg_val = readl(SUNXI_CCM_BASE+0x69C);
		reg_val &= ~(0x1<<0);
		writel(reg_val, SUNXI_CCM_BASE+0x69C);
...
{
	static char * hardware_info  = "sun50iw3p1";
