...
static void set_bmp_decode_flag(int flag)
{
	bmp_decode_flag = flag;
}

static int get_bmp_decode_flag(void)
{
...
int sunxi_secendary_cpu_task(void)
{
	...
	if (BMP_DECODE_SUCCESS == get_bmp_decode_flag()) {
		...
		tick_printf("boot logo display ok\n");
	}
...
{
	printf("check cpu power status\n");
	while(sunxi_probe_cpu_power_status(2));
	printf("cpu2 has poweroff\n");

	while(sunxi_probe_cpu_power_status(1));
	printf("cpu1 has poweroff\n");
	return 0;
