...
int download_standard_gpt(void *sunxi_mbr_buf, size_t buf_size, int storage_type)
{
	...
	int nblock = ALIGN(data_len,512)/512;
	ret = sunxi_sprite_write(0, nblock, gpt_buf);
	if(ret != nblock)
	{
		printf("write gpt %d sectors fail\n", nblock);
		return -1;
	}
	if(STORAGE_EMMC == storage_type || STORAGE_EMMC3 == storage_type)
	{
		ret = sunxi_sprite_phywrite(0, nblock, gpt_buf);
		if(ret != nblock)
		{
			printf("emmc write gpt fail\n");
			return -1;
		}
	}
