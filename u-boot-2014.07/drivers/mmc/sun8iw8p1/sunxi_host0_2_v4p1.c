...
#define  SMC_RESP_TIMEOUT     0x2
    ...
    writel(3, &reg->csdc);
#else 
    writel((512<<16)|(1U<<2)|(1U<<0), &reg->thldc); //need modify
#endif  
...
static int mmc_trans_data_by_dma(struct mmc *mmc, struct mmc_data *data)
{
    ...
    if (mmchost->mmc_no == 2)
        ...
        writel((2U<<28)|(7U<<16)|8, &reg->ftrglevel); /* burst-8, rx/tx trigger level=7/248 */
...
out:
    if(error){
        mmchost->raw_int_bak = readl(&reg->rint )& 0xbbc2;
