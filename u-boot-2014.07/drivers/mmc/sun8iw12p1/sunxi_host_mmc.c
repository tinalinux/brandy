...
static int mmc_resource_init(int sdc_no)
{
	...
	mmchost->hclkbase = CCMU_SMHC_BGR_REG;
	mmchost->hclkrst  = CCMU_SMHC_BGR_REG;
...
int get_mmc_gpio_bias(void)
{
	u32 val = ((readl(SUNXI_PIO_BASE + GPIO_POW_MODE_REG))>>2)&0x1;
	return val == 1 ? 1800:3300;
}
...
int sunxi_mmc_init(int sdc_no)
{
	...
	MMCDBG("mmc %d mmc->host_caps %x\n", sdc_no, host->cfg.host_caps);
